...
export default function ProfilesList() {
  ...
  const handleVoir = async (matricule) => {
    try {
  ...
  const tableData = useProfileTableData(handleVoir);
  ...
  const exportToPDF = () => {
    ...
    const getText = (cell) => {
      if (!cell) return "N/A";
      return typeof cell === "object" && cell.props?.children
        ? cell.props.children
        : cell.toString();
    };
    ...
    autoTable(doc, {
      startY: 50,
      head: [["Matricule", "Nom", "Prenom", "CIN", "Email", "Téléphone", "Unité"]],
      body: rows,
            ...
            {selectedProfile ? (
              <>
                <MDTypography variant="h6" gutterBottom>
                  Détails du profil: {selectedProfile.nom} {selectedProfile.prenom}
                </MDTypography>
                <MDTypography>
                  <strong>Matricule:</strong> {selectedProfile.matricule}
                </MDTypography>
                <MDTypography>
                  <strong>CIN:</strong> {selectedProfile.cin}
                </MDTypography>
                <MDTypography>
                  <strong>Fonction:</strong> {selectedProfile.fonction}
                </MDTypography>
                <MDTypography>
                  <strong>Date de naissance:</strong> {selectedProfile.dateNaissance}
                </MDTypography>
                <MDTypography>
                  <strong>Région:</strong> {selectedProfile.region}
                </MDTypography>
                <MDTypography>
                  <strong>Email:</strong> {selectedProfile.email}
                </MDTypography>
                <MDTypography>
                  <strong>Téléphone:</strong> {selectedProfile.telephone}
                </MDTypography>
                <MDTypography>
                  <strong>Unité:</strong> {selectedProfile.unite || "N/A"}
                </MDTypography>
              </>
            ) : (
