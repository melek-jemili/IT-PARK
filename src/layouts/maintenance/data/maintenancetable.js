...
export default function useMaintenanceTableData(userId, handleVoir) {
        ...
        const rows = maintenances.map((maintenance) => ({
          ...
          details: (
            <MDTypography
              ...
              onClick={() => handleVoir(maintenance.idMaintenance)}
            >
              Voir
            </MDTypography>
          ),

          // Données brutes à transmettre à row.original (invisibles mais nécessaires)
          ...
          datecréation: maintenance.datecréation,
